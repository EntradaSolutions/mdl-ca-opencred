...
export const didWebDocument = async (req, res) => {
  ...
  if(authzReqKey) {
    ...
    if(Object.keys(config.opencred.didWeb?.mainDocument).length > 0) {
      const doc = {
        ...
        verificationMethod: [
          ...
          {
            id: `${domainToDidWebEncoded(baseUri)}#${authzReqKey.id}`,
            controller: domainToDidWebEncoded(baseUri),
            type: 'JsonWebKey2020',
            publicKeyJwk: {
              ...(await exportJWK(key))
            }
        ...
        assertionMethod: [
          ...
          `${domainToDidWebEncoded(baseUri)}#${authzReqKey.id}`
        ]
      };
      res.send(doc);
      return;
    ...
    const doc = {
      ...
      id: domainToDidWebEncoded(baseUri),
      verificationMethod: [
        {
          id: `${domainToDidWebEncoded(baseUri)}#${authzReqKey.id}`,
          controller: domainToDidWebEncoded(baseUri),
